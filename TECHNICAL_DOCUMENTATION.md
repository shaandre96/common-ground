...
5. **RLS-everywhere, near-zero service-key in the app.** Every public table has `enable row level security`. The Next.js server uses the publishable (anon) key for all normal requests; the service/secret key lives in `.env.local` and is used by maintenance scripts (`seed-bots.ts`, `bot.ts`) plus exactly one server action — `deleteAccount` — which needs `auth.admin.deleteUser` to erase an account (the `auth.users → profiles → *` FK cascade then wipes all of that user's data). RLS-bypass paths (like `find_match` inserting into `matches`) are otherwise confined to specific `SECURITY DEFINER` functions.
...
CONTACT_TO_EMAIL=<optional — where /contact mail goes; defaults to shaandre96@gmail.com>
